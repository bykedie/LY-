...
- `/api/start` 本次启动账号列表必须严格校验为数组；非数组不得被当作“未指定账号”而启动全部账号。
- `/api/window` 和 `/api/send` 的目标归一化修复保持不回退。
...
- `/api/start` 非数组 `accounts` 拒绝已修复并完成完整验证，已保存为最新本地提交。
- CSS 重复选择器、`!important` 和大型文件已建立不得继续增长的维护基线。

## 正在进行事项

`/api/start` 非数组 `accounts` 启动边界问题已完成真实复现、修复前失败回归、最小修复、交接校验、维护校验、安全审计、完整测试和本地里程碑提交。当前工作区应保持干净；下一轮继续审计高风险运行期边界。

## 下一步明确动作

继续审计 Dashboard/执行端运行期控制命令是否还有可复现的用户可见状态语义问题；优先检查运行控制 API 的输入形状、目标选择、停止/初始化状态与执行端回执是否会产生用户可见误导，并按“真实复现 → 失败回归 → 最小修复 → 完整验证 → 本地提交”推进。

## 已修改文件

无未提交修改。本轮启动账号列表类型修复已保存为最新本地提交。

## 未解决问题和阻塞项

- `src/index.js` 维护预算剩余很少，当前为 2222/2225；后续触及执行端逻辑时优先提取小模块。
- `src/dashboard.js` 当前为 786/800；后续触及 Dashboard 时优先提取小模块，避免继续堆积启动流程。
...
- 旧实现真实复现：临时 Dashboard 与假执行端启动后调用 `/api/start`，正文为 `{ accounts: "StartBot" }`，API 返回 HTTP 200、`running:true`，并启动执行端。
- 新回归旧实现失败：`node scripts/smoke-test.js` 失败于“非数组启动账号列表没有被拒绝”，证明旧 Dashboard 会接受非法 `accounts` 类型。
- 修复后专项 `node scripts/smoke-test.js`：通过；缺省账号列表仍启动全部，非数组 `accounts` 返回“本次启动账号列表必须是数组。”。
- `npm.cmd run handoff:check`：通过，`v1.0.40 -> v1.0.42`、`local/v1.0.42`、推送许可为否。
- `npm.cmd run maintenance:check`：通过；`src/index.js` 2222/2225，`src/dashboard.js` 786/800，`src/execution-config.js` 100/100，CSS 重复选择器 `80/80`、`!important` `12/12`。

...
- `/api/send` 发送内容必须是字符串；对象、数组、数字、布尔值等非文本不得被送入执行端。
- `/api/start`、`/api/window` 和 `/api/send` 的既有目标/账号归一化修复保持不回退。
...
- `/api/send` 非字符串 `message` 拒绝已修复并完成完整验证，已保存为最新本地提交。
- CSS 重复选择器、`!important` 和大型文件已建立不得继续增长的维护基线。

## 正在进行事项

`/api/send` 非字符串 `message` 输入形状问题已完成真实复现、修复前失败回归、最小修复、交接校验、维护校验、安全审计、完整测试和本地里程碑提交。当前工作区应保持干净；下一轮继续审计高风险运行期边界。

## 下一步明确动作

继续审计 Dashboard/执行端运行期控制命令是否还有可复现的用户可见状态语义问题；因为 `src/dashboard.js` 已达 789/800，下一轮触及 Dashboard 逻辑时优先提取小模块或把边界校验下沉到专用 helper，再按“真实复现 → 失败回归 → 最小修复 → 完整验证 → 本地提交”推进。

## 已修改文件

无未提交修改。本轮发送消息类型修复已保存为最新本地提交。

## 未解决问题和阻塞项

- `src/dashboard.js` 当前为 789/800，距离维护预算只剩 11 行；后续触及 Dashboard 时优先提取小模块，避免继续堆积启动和运行控制流程。
...
- 旧实现真实复现：临时 Dashboard 与假执行端启动后调用 `/api/send`，正文为 `{ target: "MessageBot", message: { text: "/oops" } }`，API 返回 HTTP 200、`queuedTargets:["MessageBot"]`，假执行端收到对象型 `message`。
- 新回归旧实现失败：`node scripts/runtime-config-protocol-test.js` 失败于“非文本发送内容没有被 Dashboard 明确拒绝”，证明旧 Dashboard 未在路由层拒绝对象型 `message`。
- 修复后专项 `node scripts/runtime-config-protocol-test.js`：通过；非字符串 `message` 返回“发送内容必须是文本。”，合法文本 trim、目标 trim、执行端发送回执和超时语义保持不变。
- `npm.cmd run handoff:check`：通过，`v1.0.40 -> v1.0.42`、`local/v1.0.42`、推送许可为否。
- `npm.cmd run maintenance:check`：通过；`src/index.js` 2222/2225，`src/dashboard.js` 789/800，`src/execution-config.js` 100/100，CSS 重复选择器 `80/80`、`!important` `12/12`。

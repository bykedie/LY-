...
- `/api/send` 的发送目标如果存在，必须是字符串；非字符串 `target` 不能被隐式转换为 `all` 或其他账号名。
...
- `/api/send` 非字符串 `target` 拒绝已修复并完成完整验证，已保存为最新本地提交。
- CSS 重复选择器、`!important` 和大型文件已建立不得继续增长的维护基线。

## 正在进行事项

`/api/send` 非字符串 `target` 输入形状问题已完成真实复现、修复前失败回归、最小修复、架构同步、交接校验、维护校验、安全审计、完整测试和本地里程碑提交。当前工作区应保持干净；下一轮继续审计高风险运行期边界。

## 下一步明确动作

继续审计 Dashboard/执行端运行期控制命令是否还有可复现的用户可见状态语义问题；因为 `src/dashboard.js` 为 784/800，下一轮触及 Dashboard 逻辑时继续优先提取小模块或把边界校验下沉到专用 helper，再按“真实复现 → 失败回归 → 最小修复 → 完整验证 → 本地提交”推进。

## 已修改文件

无未提交修改。本轮发送目标类型修复已保存为最新本地提交。

## 未解决问题和阻塞项

- `src/dashboard.js` 通过提取聊天命令 helper 后为 784/800，仍接近维护预算；后续触及 Dashboard 时继续优先小模块拆分。
...
- 旧实现真实复现：临时 Dashboard 与假执行端启动后调用 `/api/send`，正文为 `{ target: false, message: "/chat-accept" }`，API 返回成功并报告 `queuedTargets:["all"]`，假执行端收到 `target:"all"`。
- 新回归旧实现失败：`node scripts/runtime-config-protocol-test.js` 失败于“非文本发送目标没有被 Dashboard 明确拒绝”，证明旧 Dashboard 未在路由层稳定拒绝非字符串 `target`。
- 修复后专项：`node --check src/runtime-chat-command.js`、`node --check src/dashboard.js` 和 `node scripts/runtime-config-protocol-test.js` 通过；新增回归覆盖非字符串 `target`，既有 target trim、非字符串 message、发送回执、窗口快照和配置热更新场景保持不变。
- `npm.cmd run handoff:check`：通过，`v1.0.40 -> v1.0.42`、`local/v1.0.42`、推送许可为否。
- `npm.cmd run maintenance:check`：通过；`src/index.js` 2222/2225，`public/app.js` 1728/1800，`src/dashboard.js` 784/800，CSS 重复选择器 `80/80`、`!important` `12/12`。

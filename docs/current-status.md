...
`v1.0.42`，业务代码发布点为 `8f37dde`；远端 `origin/main` 已包含该发布点。管理脚本菜单、Bootstrap 版本和期望管理脚本版本已同步为 `v1.0.42`。
...
保持已发布业务基线为 `v1.0.42`，仅发布同版本号的 Ubuntu 一键管理脚本展示与校验；发布后立即封存并停止，不发布本地 `v1.0.43` 功能。
...
正在发布管理脚本 `v1.0.42` 版本同步；业务代码仍保持既有稳定基线。

## 下一步明确动作

验证并推送管理脚本版本同步；发布完成后恢复推送许可为否，并等待下一项明确需求。

## 已修改文件

本轮仅修改 `deploy/bootstrap.sh`、`deploy/ly-afk-manager.sh`、稳定版 smoke 断言和交接记录；未包含本地 `v1.0.43` 功能。
...
- 当前无外部代码阻塞；GitHub 连接曾连续三次失败，本轮推送前必须重新确认远端。
...
否。项目所有者已在本轮消息中明确授权本次管理脚本版本同步推送；该一次性授权不改变长期默认值，发布完成后自动失效。

## 最后更新时间

2026-07-26 20:02:30 +08:00

...
- `/api/send` 指定账号目标应与其他目标入口一致进行首尾空白归一化，避免合法账号被误报不存在。
...
- `/api/send` 指定账号目标空白归一化已修复并完成完整验证，已保存为最新本地提交。
- CSS 重复选择器、`!important` 和大型文件已建立不得继续增长的维护基线。

## 正在进行事项

`/api/send` 指定账号目标空白归一化问题已完成真实复现、修复前失败回归、最小修复、交接校验、维护校验、安全审计、完整测试和本地里程碑提交。当前工作区应保持干净；下一轮继续审计高风险运行期边界。
...
无未提交修改。本轮发送目标 trim 修复已保存为最新本地提交。

## 未解决问题和阻塞项

- `src/index.js` 维护预算剩余很少，当前为 2222/2225；后续触及执行端逻辑时优先提取小模块。
- `src/dashboard.js` 当前为 784/800，仍在预算内但后续也应避免继续堆积启动流程。
...
- 旧实现真实复现：临时 Dashboard 与假执行端启动后调用 `/api/send`，正文为 `{ target: " TrimBot ", message: "/trimmed" }`，API 返回 HTTP 400 和“发送目标不存在或未启用： TrimBot ”。
- 新回归旧实现失败：`node scripts/runtime-config-protocol-test.js` 失败于“发送目标不存在或未启用： ConfigProtocolBot ”，证明旧 Dashboard 没有归一化 `/api/send` 指定账号目标。
- 修复后专项 `node scripts/runtime-config-protocol-test.js`：通过；`/api/send` 指定账号目标会 trim 首尾空白后发送，`all` 默认目标、空消息拒绝、禁用账号拒绝和执行端发送回执语义保持不变。
- `npm.cmd run handoff:check`：通过，`v1.0.40 -> v1.0.42`、`local/v1.0.42`、推送许可为否。
- `npm.cmd run maintenance:check`：通过；`src/index.js` 2222/2225，`src/dashboard.js` 784/800，`src/execution-config.js` 100/100，CSS 重复选择器 `80/80`、`!important` `12/12`。

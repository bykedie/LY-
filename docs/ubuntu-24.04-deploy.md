...
一键部署填写参数时，每一项都会显示说明和示例。这里最容易混淆的是“监听地址”：

```text
127.0.0.1：只允许服务器本机访问，推荐配合 Nginx 域名访问。
0.0.0.0：允许公网端口直连，例如 http://当前公网IP:30123。
```

你可以选择：

```text
1. 仅域名访问：更推荐，端口不直接暴露公网。
2. 仅公网端口访问：适合临时测试。

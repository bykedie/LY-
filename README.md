...
```bash
curl -fL https://cdn.jsdelivr.net/gh/bykedie/LY-@main/deploy/bootstrap.sh | sudo INSTALL_DIR=/opt/pcl-afk-bot bash
```
...
如果 jsDelivr 也访问失败，可以改用 GitHub raw 源：

```bash
curl -fL https://raw.githubusercontent.com/bykedie/LY-/main/deploy/bootstrap.sh | sudo bash
```
...
一键部署会在每个输入项前显示说明和示例。访问方式支持三种选择：仅域名访问、仅公网端口访问、域名 + 公网端口共存。监听地址也会自动按你的选择写入：`127.0.0.1` 适合域名反代，`0.0.0.0` 适合公网端口直连。
...
- 功能综合区：集中配置战斗挂机、移动辅助和智能交互。
- 战斗挂机：支持自动攻击、目标范围、攻击间隔、黑白名单和自动重生。
- 自动钓鱼：支持自动抛竿收竿。
- 移动辅助：支持自动走路、防挂机随机移动、潜行和切换手持物品。
- 智能交互：支持关键词自动回复、预设消息、远程发送聊天和命令。
- 大厅功能：支持可持久化动作方案、当前坐标与背包窗口读取、定向移动、寻找并交互可见 NPC、按物品名点击菜单、协议级槽位移动、等待聊天内容和执行聊天 `clickEvent` 按钮。
...
- 协议流程：启动一个本地测试 Minecraft 协议服务端，验证机器人能进服、自动注册、关键词回复、登录后定时任务、网页指令通道、防挂机命令、切换手持、自动走路、大厅动作以及聊天 `clickEvent` 自动执行。

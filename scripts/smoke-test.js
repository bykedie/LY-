...
try {
  ...
  assert(!page.includes('value="clickChat"'), '聊天按钮不应继续作为独立动作类型显示');
  assert(!page.includes('value="clickNpcDialog"'), 'CustomNPCs 不应继续作为独立动作类型显示');
  ...
  const uninstallScript = fs.readFileSync(path.join(projectRoot, 'deploy', 'uninstall.sh'), 'utf8');
  const uninstallTest = fs.readFileSync(path.join(projectRoot, 'scripts', 'uninstall-script-test.sh'), 'utf8');
  ...
  assert(managerScript.includes('v1.0.43'), '管理脚本版本没有同步待发布稳定版 v1.0.43');
  assert(bootstrapScript.includes('BOOTSTRAP_VERSION="v1.0.43"'), '启动脚本版本没有同步待发布稳定版 v1.0.43');
  assert(bootstrapScript.includes('EXPECTED_MANAGER_VERSION="v1.0.43"'), '启动脚本期望版本没有同步待发布稳定版 v1.0.43');
  ...
  assert(bootstrapScript.includes('chmod +x deploy/ly-afk-manager.sh deploy/uninstall.sh'), '首次部署没有设置卸载脚本执行权限');
  ...
  assert(managerScript.includes('16. 一键卸载 LY 控制台'), '管理脚本缺少一键卸载菜单');
  assert(managerScript.includes('run_uninstall'), '管理脚本缺少统一卸载入口');
  assert(managerScript.includes('sudo env INSTALL_DIR="$PROJECT_DIR" bash "$PROJECT_DIR/deploy/uninstall.sh"'), '管理脚本卸载入口没有提权并传入当前项目目录');
  assert(managerScript.includes('"$PROJECT_DIR/deploy/ly-afk-manager.sh" "$PROJECT_DIR/deploy/uninstall.sh"'), '项目更新没有处理卸载脚本执行权限');
  assert(uninstallScript.includes('请输入 UNINSTALL 确认卸载'), '卸载脚本缺少明确的二次确认');
  assert(uninstallScript.includes('dir_has_ly_markers'), '卸载脚本缺少项目目录识别保护');
  assert(uninstallScript.includes('detect_install_dir_from_shortcut'), '卸载脚本没有从快捷命令恢复自定义安装目录');
  assert(uninstallScript.includes('grep -Fqx "cd \\"${INSTALL_DIR}\\""'), '卸载脚本没有核对 j 快捷命令所属安装目录');
  assert(uninstallScript.includes('拒绝卸载包含符号链接的路径'), '卸载脚本缺少符号链接路径保护');
  assert(uninstallScript.includes('/var/backups/ly-console'), '卸载脚本缺少默认运行数据备份目录');
  assert(uninstallScript.includes('SERVICE_NAME="ly-afk-dashboard"'), '卸载脚本没有锁定项目 PM2 服务名');
  assert(uninstallScript.includes('delete "$SERVICE_NAME"'), '卸载脚本没有删除项目 PM2 服务');
  assert(uninstallScript.includes('remove_service_from_pm2_dump_file'), '卸载脚本没有清理 PM2 持久化清单');
  assert(uninstallScript.includes('pm2_daemon_running'), '卸载脚本没有区分 PM2 守护进程是否运行');
  assert(uninstallScript.includes('/etc/nginx/sites-available/${NGINX_SITE_NAME}'), '卸载脚本没有清理项目 Nginx 站点');
  assert(uninstallScript.includes('verify_uninstall'), '卸载脚本没有核对项目专属资源是否清理完成');
  assert(uninstallScript.includes('rm -rf --one-file-system'), '卸载脚本删除项目目录时可能跨越挂载文件系统');
  assert(!uninstallScript.includes('apt remove'), '卸载脚本不应删除可能共享的系统软件包');
  assert(uninstallTest.includes('普通目录不应被识别为 LY 控制台'), '卸载脚本专项测试缺少非项目目录保护');
  assert(uninstallTest.includes('符号链接安装路径不应被接受'), '卸载脚本专项测试缺少符号链接路径保护');
  assert(uninstallTest.includes('PM2 持久化清单仍包含待卸载服务'), '卸载脚本专项测试缺少 PM2 持久化清单清理');
